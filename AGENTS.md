...
| `docs/context/ai_work_logs/<date>_<task_id>_<summary>.md`（AI作業ログ。正式仕様ではない。AIは明示指示なしに作成・更新しない） | `docs/templates/ai_work_log_template.md` |
...
ソースコードを読んでいて変更を思いついた時点では、それがバグなのか、仕様変更なのか、設計改善・リファクタリング・類似機能間の整合性改善・標準化・共通化候補なのかが未確定な場合があります。

- 仕様どおり正常に動作していても、内部構造を改善したい場合がある
- 類似機能間で実装方式が微妙にズレていて、そろえたい場合がある

分類が未確定なまま作業を始めず、まず `prompts/analyze_code_change_impact.md` で影響範囲を整理してください。

- この分析ではファイルを変更しない
- 影響する正式資料を列挙するだけでなく、変更内容を定義すべき最上流かつ役割の合う資料を特定し、それを変更内容の正本とする
- 正本から下流へ順番に反映する。上流資料を形式的にすべて変更せず、変更不要な資料は理由を明示する
- 適切な正本が現在の構成に存在しない場合は、AIが新しい資料や配置場所を勝手に作らず、人間判断に回す
- 分析結果を人間が確認し、正式な作業フローを選ぶ
- バグ候補なら「バグ修正の基本ルール」のフローへ進む
- 仕様変更なら「仕様変更時の作業ルール」へ進む
- 共通化候補でも、「common の扱い」に従い、勝手に `src/common/` を変更しない
...
更新は、上流資料から下流へ順番に進めてください。

- command/app の責務や feature 分割へ影響する場合は、`10_overview.md` から確認・更新する
- 外部から見える動作へ影響する場合は、設計書や実装より先に `20_spec.md` を更新する
- その後、設計（`21_design.md`）、フロー（`22_flow.md`）、テスト計画（`23_test_plan.md`）、実装、テストの順に進む
- 変更不要な上流資料は更新しない（更新不要と判断した理由は下記のとおり報告する）

どの資料を変更内容の正本とするかの判断は、「コード起点の変更検討」で参照する `prompts/analyze_code_change_impact.md` に整理されています。
