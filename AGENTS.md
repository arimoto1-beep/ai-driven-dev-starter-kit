...
- `tasks.md` の AI 更新方針はハイブリッド方式に従う
  - **ドキュメント作成工程**（overview 作成、仕様作成、設計作成、フロー作成、テスト計画作成、レビュー観点作成、レビュー結果作成など）では、現在地メモとして最小限の更新を行ってよい
  - **実装工程・バグ修正工程**（feature 実装、entrypoint 実装、結合試験実装、バグ報告書作成、バグ調査書作成、バグ修正計画書作成、バグ修正実装など）では、`tasks.md` を直接更新しない。更新が必要な場合は、作業報告に「tasks.md 更新候補」として記載し、人間が反映を判断する
...
レビュー補助メモは、AIが成果物（ドキュメントや実装）を作り終えたときに、人間レビューを速くするために作業完了報告へ添える短い申し送りです。

目的は、人間がレビューに入る前に「どこを重点的に見ればよいか」を素早く把握できるようにすることです。
レビュー補助メモは、AIの思考過程や試行錯誤の記録ではありません。レビュー判断に必要な結論だけを短く残します。

レビュー補助メモには、次の5項目を簡潔に記載します。該当がない項目は「なし」と書きます。

1. 判断が分かれそうだった点 — レビューで意見が割れそうな箇所
2. 採用した判断と理由 — その箇所で今回採用した方針と、採用した理由
3. 採用しなかった案 — 検討したが採用しなかった選択肢と、採用しなかった理由
4. 仮置きした前提 — 確定情報がなく、仮に置いた前提
5. 人間に確認してほしい点 — レビューで明示的に確認してほしい点

記載ルール:

- 各項目は数行までにとどめ、結論を箇条書きで書く。検討の経緯や試行錯誤のログは書かない。
- 特定の会社・案件・担当者・社内用語・汎用キット外の内部管理番号など、汎用キット外の情報を持ち込まない。
- レビュー補助メモに合否判定（`OK` / `要修正` など）は書かない。最終判定は `25_review_result.md` / `12_command_review_result.md` の領分とする。
- レビュー補助メモは作業完了報告（チャット）に出力する。`tasks.md` には書かない。専用ファイルも作らない。
...
仕様変更を反映する場合、設計書・ソースコード・テストを一括でまとめて修正しないでください。

AIは、まず仕様変更の内容を整理し、影響範囲を洗い出してください。
そのうえで、設計書・ソースコード・テストの更新要否を資産ごとに確認してください。
...
作業完了時は、通常の作業完了報告に加えて、以下を明記してください。

- 更新したファイル
- 更新しなかったファイル
- 更新不要と判断した理由
- 設計書・ソースコード・テストの整合性確認結果
- 実行した確認
- 残課題

更新不要と判断した資産がある場合でも、理由を省略しないでください。
...
- AIへの依頼内容・確認結果・更新有無などを記録するAI作業ログは `docs/context/ai_work_logs/` に置く（詳しくは `docs/context/ai_work_logs/README.md` を参照）。AI作業ログも正式仕様ではなく確定仕様として扱わず、AIは人間の明示指示なしに作成・更新しない。正式反映が必要な内容は、仕様書・設計書・テスト計画・レビュー結果などへ別作業として反映する
...
利用者が明示的にルール更新を依頼した場合のみ、`AGENTS.md` や `prompts/*.md` を必要最小限で更新してください。
...
「## レビュー補助メモ」の定義に従って、以下を簡潔に記載する（該当がない項目は「なし」）。
