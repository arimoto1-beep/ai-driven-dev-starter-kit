...
上の2つの手順は、`24_review_checklist.md` の「実装開始条件」が `manual`（既定）の場合のものです。`auto` の場合は実装着手承認欄を使用せず、**CP1 の仕様承認（現在の `20_spec.md` と同一 baseline）AND 最新 G2 Gate記録の `verdict: PASS`** が実装開始条件になります（`docs/rules/project/70_feature_loop.md`）。
...
- **実装詳細で完結する変更（ルートA）でも、実装開始条件の確認は通常どおり必要です。** ルートAであることは、承認ゲートを省略する理由になりません（方式が `manual` なら実装着手承認欄、`auto` なら `docs/rules/project/70_feature_loop.md` の条件）
