...
- コードに関する変更を、「正式資料で意味を定義・維持する必要があるか」で整理する流れ（変更前でも変更後でも使えます）
- 正式資料に「書かれていないこと」と「矛盾していること」を分けて扱う流れ
...
│  ├─ review_command.md
│  ├─ review_context.md
│  ├─ review_design_code_consistency.md  ← 正式資料とコードの意味上の整合を確認する
│  ├─ analyze_code_change_impact.md      ← 変更の意味と正式資料への反映要否を整理する
...
**変更は、必ずしも仕様書から始まりません。** このキットでは、次を中心に判断します。

```text
この変更は、正式資料で意味を定義・維持する必要があるか。
```

「どこから変更を始めたか」や「人間が直したか、AIが直したか」を中心にはしません。判断するのは、その変更が持つ意味です。

判断には2つの異なる軸を使います。**変更ルート**（正式資料でどう扱うべきか）と、**実施状態**（未着手・一部実施・実施済み、コード先行の有無）です。この2つは同時に成立します。たとえば「今後も守るべき方針だと分かって正式資料へ反映する」変更を、コードが先に直っている状態で行うこともあります。**コード先行は独立した変更ルートではなく、実施状態です。**

少なくとも次の変更ルートがあります。

| ルート | 内容 | 進み方 |
|---|---|---|
| **A. 実装詳細で完結する変更** | 可読性の改善、冗長な処理の整理、局所的なリファクタリングなど、正式資料が意味を定義していない範囲だけが変わる | 正式資料を無理に更新しない。コードとテストの整合・検証・レビュー・報告は行う |
| **B. 正式資料から下流へ反映する変更** | 外部から見える動作、責務、制約、処理フロー、共通設計など、正式資料で定義すべき意味が変わる | 正本を特定し、そこから下流へ反映する（従来どおり） |
| **C. コードから正式資料へ逆反映する変更** | コードを読んだ結果、今後も守るべき仕様・設計・共通方針だと分かった | 役割の合う正式資料へ、別作業として反映する |
| **判断不能** | 上記を確定できない | 人間判断へ戻す |

ルートB・Cの変更が、正式資料への反映より先にコードで実施されている場合（**コード先行**）は、後から正式資料へ反映し、必要なテストとレビューを行います。

**「正式資料に書かれていない実装詳細が存在すること」自体は、不整合ではありません。** 一方で、**正式資料が定義している意味と矛盾するコード**は、従来どおり問題として扱います。この2つを混同しないでください。

### 人間がコードを直接修正してもかまいません

人間がソースコードを直接修正することは禁止されていません。修正した後で、その変更が上記のどの変更ルートに当たるかを確認します。**このために、新しい承認工程や毎回の記録作成といった重い運用は設けていません。**

**ただし、これはAIの変更権限を広げるものではありません。** AIは引き続き、その task で許可された範囲だけを変更します。調査中・レビュー中の修正、ついで修正、保護対象の変更、人間判断事項の確定は、従来どおり行いません。

### 判断を支援する2つのプロンプト

| プロンプト | 見るもの | 使う場面 |
|---|---|---|
| `prompts/analyze_code_change_impact.md` | **変更** | この変更をどこまで正式な意味として残すか整理したいとき。**変更前でも変更後でも使えます** |
| `prompts/review_design_code_consistency.md` | **現在の状態** | 今の正式資料と今のコードに、意味上の矛盾がないか確認したいとき |

どちらもファイルを変更せず、結果をチャットで報告します。採否と次に進むフローは人間が判断します。

`analyze_code_change_impact.md` は、実施済みの変更（Git差分）も入力にできます。人間が直接行った変更でも、AIが行った変更でも同じように扱います。分析結果では、変更点ごとに上記の変更ルートと根拠、実施状態を示します。**「正本が見つからない」ことだけを理由に一律で止めることはしません。** ただし、本来正式資料へ残すべき意味なのに置き場がない場合は、従来どおり人間判断に戻します。

バグ候補と分類された場合はバグ修正フローへ進みます。共通化候補の場合も、AIが `src/common/` を勝手に変更することはありません。

**詳しい判断基準の正本は `docs/rules/core/10_workflow.md` です。** 上記は概要です。
...
`prompts/review_design_code_consistency.md` は、指定した scope（feature / command/app / 共通処理など）について、**現在の正式資料と現在のコードに意味上の矛盾がないか**を確認する随時利用のプロンプトです。上記3つのレビューを置き換えるものではありません。ファイルは変更しません。

いずれのレビューでも、**正式資料に書かれていない実装詳細が存在すること自体は指摘しません。** 指摘するのは、正式資料が定義している意味とコードが食い違っている場合です。ただし、仕様にない便利機能、外部動作の変更、責務としての意味の変更などは「未記載だから問題なし」とはしません。
...
| `030_update_existing_feature.md` | 実装済み feature に対して、仕様変更・軽微な機能追加・内部設計の改善・リファクタリング・類似機能間の整合性改善・レビュー指摘反映を安全に行う流れを確認する。変更の意味と正式資料への反映要否は `analyze_code_change_impact.md` で整理する。バグ修正は 040 を参照。 |
